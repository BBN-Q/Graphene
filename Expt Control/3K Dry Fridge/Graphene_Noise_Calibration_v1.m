...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [data] = Graphene_Noise_Calibration_v1(T_Array,Vg_Array,Vex)
tic
% Initialize the path and create the data file with header\
Rex=9.94E6; disp('Setting Rex to 9.94 MOhm'); %used to calculate R from Vsd
...
if Vex>Vex_max
    error('Vex is set above Vex_max')
end
...
for T_n = 1:length(T_Array)
    ...
    if T_n>1&&T_Array(T_n)~=T_Array(T_n-1)
        pause(TWaitTime);
    end
    ...
    for Vg_n = 1:length(Vg_Array)
        ...
        for n=1:Nmeasurements
            ...
            data.raw.R(T_n,Vg_n,n)=VsdR*Rex/(Vex-VsdR);
            ...
            tmp=[CurrentTemp,CurrentVg,Vex,VsdX,VsdY,VnX,VnY];
    ...
    %plot
    figure(91);hold on;xlabel('Gate Voltage (V)');ylabel('Resitance (Ohm)');
    plot(data.Vg(end,:),data.R(end,:));
    
    figure(92);hold on;xlabel('CryoCon Temperature (K)');ylabel('Lock-in Voltage (V)');
    plot(data.CryoT(:,1),data.VnX(:,1));
