    ...
    for Vg_n = 1:length(Vg_Array)
        ...
        for Vex_n=1:length(Vex_Array)
            ...
            for n=1:Nmeasurements
                ...
                data.VnoiseX(CurrentMeasurementNumber)=CurrentVnoise(1);
                data.VnoiseY(CurrentMeasurementNumber)=CurrentVnoise(2);
                
                %save results to file
                tmp=[CurrentTemp,CurrentVg,CurrentVex,CurrentVsd(1)...
                    ,CurrentVsd(2),CurrentVnoise(1),CurrentVnoise(2)];
            ...
            data.AveVnoiseX(CurrentParameterSet)=mean(data.VnoiseX(end-Nmeasurements+1:end));
            data.StdVnoiseX(CurrentParameterSet)=std(data.VnoiseX(end-Nmeasurements+1:end));
            data.AveVnoiseY(CurrentParameterSet)=mean(data.VnoiseY(end-Nmeasurements+1:end));
            data.StdVnoiseY(CurrentParameterSet)=std(data.VnoiseY(end-Nmeasurements+1:end));
