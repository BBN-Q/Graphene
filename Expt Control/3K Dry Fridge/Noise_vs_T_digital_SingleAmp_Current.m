...
fprintf(FilePtr, strcat(datestr(StartTime), ' Noise vs. Temperature using CryoCon\r\n'));
...
for m = 1:length(SetTArray)
    ...
    for k=1:100
        ...
        XCNoiseData(j,:) = [XCNoiseStatistics.Temperature(j) mean(DoubleTraces(:,3).*DoubleTraces(:,3))];
        fprintf(FilePtr,'%f\t%e\r\n', XCNoiseData(j,:));
        j = j+1;
    end
    fclose(FilePtr);
    ...
    plot(XCNoiseData(:,1), XCNoiseData(:,2)); grid on; xlabel('T_{CryoCon} (K)'); ylabel('V_{xc} (V)'); title(strcat('Noise_', datestr(StartTime, 'yyyymmdd_HHMMSS'),'_',UniqueName));
