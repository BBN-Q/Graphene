...
for m = 1:length(SetTArray)
    ...
    for k=1:2
        ...
        TrunDoubleTraces(:,2) = DoubleTraces(1:end-TruncatedPts,2) - XCNoiseStatistics.ChAMean(j);
        TrunDoubleTraces(:,3) = DoubleTraces(1:end-TruncatedPts,3) - XCNoiseStatistics.ChBMean(j);
        XCNoiseData(j,:) = [XCNoiseStatistics.Temperature(j) dot(TrunDoubleTraces(:,2).*TrunDoubleTraces(:,3), squarewave(FreqMod, SamplingRate, TracesLength))/length(DoubleTraces)];
        fprintf(FilePtr,'%f\t%e\r\n', XCNoiseData(j,:));
        j = j+1;
    end    
    fclose(FilePtr);
