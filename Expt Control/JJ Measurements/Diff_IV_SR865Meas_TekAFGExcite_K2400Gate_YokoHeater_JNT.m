...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function Diff_IV_data = Diff_IV_SR865Meas_TekAFGExcite_K2400Gate_YokoHeater_JNT()
temp = instrfind;
if ~isempty(temp)
    fclose(temp)
    delete(temp)
...
Diff_IV_data=struct('V_Gate_Array',[],'V_Heat_Array',[],'JJCurr_Array',[],'LockInX',[],'LockInY',[],'LockInR',[],'LockInTH',[],'AFG_Excite_Volt',AFGExcitVolt,'AFG_Load_Resistor',LoadResistor,'LockIn_Time_Constant',Lockin.timeConstant(),'Measurement_Wait_Time',WaitTime,'Heater_Resistor',HeaterResistor,'Freq',[],'Spec',[],'RBW',SA.SAGetRBW,'VBW',SA.SAGetVBW);

FileName2 = strcat('Diff_IV_vs_VGHEAT_Spec_', datestr(StartTime, 'yyyymmdd_HHMMSS'), '.mat');
...
for i=1:GateSteps
    ...
    for k=1:HeatSteps
        ...
        [Diff_IV_data.Freq, Diff_IV_data.Spec(i,:,k)] = SA.SAGetTrace();
...
%%%%%%%%%%%%%%%%%%%%%       Clear     %%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Lockin.disconnect(); K2400.disconnect(); AFG.disconnect; Yoko.disconnect(); SA.disconnect;
clear SetVolt;
clear Lockin;
clear K2400;
clear Yoko;
