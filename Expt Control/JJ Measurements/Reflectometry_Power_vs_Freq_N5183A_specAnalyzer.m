...
ProbeSource.connect('128.33.89.88') %input RF source

%%%%%%%%%%%%%%%%%%%%%%        PLOT DATA         %%%%%%%%%%%%%%%%%%%%%%%%%%%

function plot_data()
    figure(237); clf;
    plot(1e-9*ProbeFreqList(1:k), 1e9*data.Spec(1:k,round(length(result.Spec)/2)))
    xlabel('Frequency (GHz)'); ylabel('Power (nW)')
end
...
for k=1:length(ProbeFreqList)
    ...
    SpecAnalyzer.connect('128.33.89.3');
