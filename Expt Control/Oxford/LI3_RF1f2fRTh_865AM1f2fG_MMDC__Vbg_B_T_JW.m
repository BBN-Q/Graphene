...
%          internal 865 params
%          July 2016: added thermal conductance measurement on 865,
%          GPIBwait time parameter
% 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function data = LI3_RF1f2fRTh_865AM1f2fG_MMDC__Vbg_B_T_JW(Vg_list,B_list,T_list)
...
%             subplot(pr,pc,7);
    ...
    %         set(h,'linestyle','none');colorbar;title('LA865 1f R (V)');
            ...
            xlabel('Gate Voltage (V)');ylabel('T (K)');
            hold on;
            pcolor(data.Vg,data.T_set,squeeze(data.raw.LA865_G2f_R(:,j,:)));shading interp;
                        colorbar;title('LA865 G 2f R (V)');
...
%             subplot(pr,pc,7);
    ...
    %         title('LA865 1f R (V)');
            ...
            xlabel('Gate Voltage (V)');
            grid on; hold on;
            h=plot(data.Vg,squeeze(data.raw.LA865_G2f_R(i,j,:)));
            title('LA865 G 2f R (V)');
...
%measurements
    function measure_data(i,j,k)
        ...
        for n=1:Nmeasurements
            ...
            LA865.sineFreq=LA_freq; 
            LA865.intFreq=LA_freq; 
            LA865.refsource=1; %external
            LA.ramp2V(LA_Vex);
            
            pause(measurementWaitTime+LA865_sensWaitTime)
            
            [data.raw.LA865_G2f_R(i,j,k,n) data.raw.LA865_G2f_theta(i,j,k,n)] = LA865.snapRtheta();
            [data.raw.LA865_G2f_R(i,j,k,n) data.raw.LA865_G2f_theta(i,j,k,n) data.settings.LA865.GsensLog] =...
                checkLockinSensitivity(LA865,data.raw.LA865_G2f_R(i,j,k,n),data.raw.LA865_G2f_theta(i,j,k,n),data.settings.LA865.GsensLog);
            ...
            data.raw.R(i,j,k,n) = data.raw.LA_R(i,j,k,n)*LA_Rex/(LA_Vex-data.raw.LA_R(i,j,k,n));
            ...
            LA865.sineFreq=SG_modrate; 
            LA865.intFreq=SG_modrate; 
            LA865.refsource=0; %internal
            LA.ramp2V(0.05);
            ...
            data.raw.R_hot(i,j,k,n) = data.raw.LA_R_hot(i,j,k,n)*LA_Rex/(0.05-data.raw.LA_R_hot(i,j,k,n));                      
...
LA_Rex =  99.3e3; %resistor in series with sample 9.79E6;
LA_Vex = 0.5; %Voltage to use on LA sine output
% LA_Vheat = 1; %Voltage to use on LA sine output for heating
...
LA865_sineFreq=SG_modrate; % now changed to include G measurement; see measure data
LA865_intFreq=SG_modrate;  % now changed to include G measurement; see measure data
LA865_refsource=0;
...
data = struct('time',blank,'TVapor',blank,'TProbe',blank,'LA_R',blank ...
    ...
    ,'LA865_G2f_R',blank,'LA865_G2f_theta',blank);
...
data.settings.GPIBwait=GPIBwait;
...
data.settings.LA865.GsensLog = LA865_sens;
%Temperature
