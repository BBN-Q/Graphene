...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function data = Noise_AM865_R_RF1f2f_T__T_B_Vg_JW(T_list,B_list,Vg_list)
%%
...
%     end
    ...
    function plotResistance(i)
        ...
        set(h,'linestyle','none');colorbar;title('RFLA 1f R (V)');
        
        figure(774); clf; xlabel('Gate Voltage (V)');ylabel('Field (Tesla)');
        grid on; hold on;
        h=surf(data.Vg,data.field_set,squeeze(data.raw.RFLA_theta_1f(i,:,:)));view(2);
        set(h,'linestyle','none');colorbar;title('RFLA 1f theta (deg)');
        
        figure(883); clf; xlabel('Gate Voltage (V)');ylabel('Field (Tesla)');
        grid on; hold on;
        h=surf(data.Vg,data.field_set,squeeze(data.raw.RFLA_R_2f(i,:,:)));view(2);
        set(h,'linestyle','none');colorbar;title('RFLA 2f R (V)');
        
        figure(884); clf; xlabel('Gate Voltage (V)');ylabel('Field (Tesla)');
        grid on; hold on;
        h=surf(data.Vg,data.field_set,squeeze(data.raw.RFLA_theta_2f(i,:,:)));view(2);
        set(h,'linestyle','none');colorbar;title('RFLA 2f theta (deg)');
        
        figure(885); clf; xlabel('Gate Voltage (V)');ylabel('Field (Tesla)');
        grid on; hold on;
        h=surf(data.Vg,data.field_set,squeeze(data.raw.LA865_R(i,:,:)));view(2);
        set(h,'linestyle','none');colorbar;title('LA865 R (V)');
        
         figure(1111); clf; xlabel('Gate Voltage (V)');ylabel('Field (Tesla)');
        grid on; hold on;
        h=surf(data.Vg,data.field_set,squeeze(data.raw.MMDCnoheat(i,:,:)));view(2);
        set(h,'linestyle','none');colorbar;title('MMDC heating off (Volt)');
        
        figure(1112); clf; xlabel('Gate Voltage (V)');ylabel('Field (Tesla)');
        grid on; hold on;
        h=surf(data.Vg,data.field_set,squeeze(data.raw.MMDCheat(i,:,:)));view(2);
...
%             pause(measurementWaitTime)
            ...
            checkLockinSensitivity(LA865);
            
                pause(measurementWaitTime)
            
            [data.raw.LA865_R(i,j,k,n) data.raw.LA865_theta(i,j,k,n)] = LA865.get_Rtheta();
            
                pause(measurementWaitTime)
            
            data.raw.MMDCheat(i,j,k,n) = MMDC.value; 
            ...
            data.raw.MMDCnoheat(i,j,k,n) = MMDC.value; 
...
FileName2 = strcat('Noise_AM865_R_RF1f2f_T__T_B_Vg_JW', datestr(StartTime, 'yyyymmdd_HHMMSS'),'_',UniqueName,'.mat');
...
data = struct('time',blank,'TVapor',blank,'TProbe',blank,'LA_X',blank ...
    ...
    ,'MMAC',blank...
    ,'LA865_R',blank,'LA865_theta',blank);
data.raw = struct('time',blank,'TVapor',blank,'TProbe',blank,'LA_X',blank ...
    ...
    ,'MMAC',blank...
    ,'LA865_R',blank,'LA865_theta',blank);
data.log = struct('time',[],'TVapor',[],'TProbe',[],'LA_X',[],'LA_Y',[],'R',[],'field',[],'RFLA_R_2f',[],'RFLA_theta_2f',[],'MMDC',[]);
...
MMDC.disconnect();
LA865.disconnect();
clear TC LA MS VG RFLA MMDC LA865
% %% Email data
