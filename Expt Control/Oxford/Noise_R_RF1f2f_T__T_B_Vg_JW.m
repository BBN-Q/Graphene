...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function data = Noise_R_RF1f2f_T__T_B_Vg_JW(T_list,B_list,Vg_list)
%%
...
%     end
    ...
    function plotResistance(i)
        ...
        set(h,'linestyle','none');colorbar;title('RFLA 1f theta (\Omega)');
        ...
        set(h,'linestyle','none');colorbar;title('RFLA 2f theta (\Omega)');
        
         figure(1111); clf; xlabel('Gate Voltage (V)');ylabel('Field (Tesla)');
        grid on; hold on;
        h=surf(data.Vg,data.field_set,squeeze(data.raw.MMDC(i,:,:)));view(2);
        set(h,'linestyle','none');colorbar;title('MMDC (Volt)');
...
%             pause(measurementWaitTime)
            ...
            data.raw.MMDC(i,j,k,n) = MMDC.value; 
...
StartTime = clock;
FileName = strcat('Noise_R_RF1f2f_T__T_B_Vg_JW', datestr(StartTime, 'yyyymmdd_HHMMSS'),'_',UniqueName,'.dat');
FileName2 = strcat('Noise_R_RF1f2f_T__T_B_Vg_JW', datestr(StartTime, 'yyyymmdd_HHMMSS'),'_',UniqueName,'.mat');
