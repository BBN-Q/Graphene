...
if strfind(cntrl, 'lock')
    ...
    getinsts=[];
    for l=1:length(scan.loops)
        loopchans=smchaninst(scan.loops(l).getchan);
        if size(loopchans,1)
            getinsts=[getinsts; loopchans(:,1)];
        end
    end
    lockin=lockin(ismember(lockin, getinsts)); %dont bother with unused insts.
    lockin=lockin'; %so that FOR loops work
else
    lockin = [];
end
